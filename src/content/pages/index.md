---
layout: layouts/home.njk
title: Home
description: Culturally grounded, inclusive counselling with Valerie M. Sipos, M.C., CCC — a welcoming space for everyone across Alberta and British Columbia.
permalink: /
hero:
  badge: A welcoming space for everyone!
  heading: Becoming is both<br><em>exciting &amp; scary</em>,<br>at the same time
  subheading: A culturally grounded, inclusive counselling practice where every person — regardless of background, identity, or origin — is honoured and welcome.
  cta_primary: Book Your Free First Session
  cta_secondary: Learn About Valerie
inclusivity:
  heading: Everyone belongs here
  body: GYST Counselling is built on a foundation of respect for all peoples. Rooted in Indigenous teachings and open to every culture, background, and identity — we believe healing is a universal right. Whether you're Indigenous, a newcomer finding your footing, or anyone seeking support, this is your space.
  values:
    - Indigenous-guided practice
    - Culturally responsive
    - Newcomer-friendly
    - LGBTQ2S+ affirming
    - All faiths & beliefs
    - Accessible online counselling
about:
  label: About Valerie
  image: /uploads/V-M-S-220x300.jpg
  years: 20+
  years_label: Years in counselling<br>&amp; community work
  bio:
    - Born in The Pas, Manitoba and raised in Canada's North, the Indigenous worldview is something I hold close and continues to guide me in everything I do. Third in a line of nine children, I learned at a very young age the value of simple basic truths.
    - I define myself physically, mentally, emotionally, and spiritually — and attend to each of these parts of myself daily. When life gets complicated, I have learned to simplify by returning to core values.
    - With years at Mount Royal University coaching Indigenous students and eight years at Hull Services supporting families healing from intergenerational trauma, I bring deep experience in culturally responsive care to my private practice. I'm a proud grandmother to six beautiful grandchildren who are the loves of my life.
  credentials:
    - title: M.C.
      description: Master of Counselling
    - title: CCC
      description: Canadian Certified Counsellor
    - title: B.A.
      description: Psychology, U of Lethbridge
philosophy:
  label: Counselling Philosophy
  heading: A journey that requires courage
  body: Counselling is a purposeful process that facilitates growth, direction, and wellness — it is all about change. A genuine interest in exploring what troubles you is the first brave step.
  quote: I would be honoured by your willingness to entrust me as a partner in this work.
  cards:
    - title: Collaborative
      body: Therapeutic change is the collaborative work done by counsellor and client — identifying goals as well as a plan to realize them together.
    - title: Safe &amp; Confidential
      body: A purposefully safe and confidential place where we can explore the possibilities in change without judgment.
    - title: Guided by You
      body: My intention is to be guided by your needs — meeting you where you are with genuine empathy and positive regard for your person.
approach:
  label: Therapeutic Perspective
  heading: Grounded in culture, guided by theory
  body: My therapeutic perspective is influenced at its foundation by cultural teachings and integrates three evidence-based frameworks.
  cards:
    - title: Existentialism
      body: A philosophical approach at whose core is the belief that each of us has the ability to find meaning in our lives. The counsellor works to understand you on your own subjective terms, encouraging reflection and taking responsibility for change.
    - title: Solutions-Focused Brief Therapy
      body: Concentrating on small, realistic, and sustainable goals that result in positive outcomes. You are empowered as the expert in who you are — we focus on solutions and results rather than dwelling on problems.
    - title: Family Systems Therapy
      body: Focusing on human problems from a relational perspective — we are best understood by how we interact within our systems. This explores intergenerational meanings, rules, processes, and cultural nuances.
events_section:
  label: Community Events
  heading: Upcoming gatherings
  body: Join us for workshops, circles, and community events. All are welcome — registration is open.
testimonials:
  label: Client Voices
  heading: What clients are saying
  body: Real words from real people who have walked this journey.
  items:
    - quote: Valerie has a way about her that has me leaving her office with renewed hope. I always look forward to the time with her.
      author: C.P.
    - quote: I felt instantly comfortable… she just made me feel so normal and I felt I could just trust her immediately.
      author: B.A.
    - quote: It was no time at all before we got right down to business. I have been to counselling before but it never seemed to go anywhere.
      author: W.P.
    - quote: She has a way of focusing on positive things and I leave feeling good about myself!
      author: H.G.
    - quote: Sitting with her has me feeling so safe. It's like the space is all mine and she's got my back.
      author: J.W.
    - quote: I found that Valerie was a very approachable person and gave feedback that was very relaxing.
      author: D.W.
blog_section:
  label: From the Blog
  heading: Insights &amp; reflections
  body: Thoughts on wellness, cultural identity, growth, and the journey of becoming.
contact:
  label: Get in Touch
  heading: Ready to begin?
  body: In any counselling relationship, "good fit" is essential. Both parties need to feel like they will work well together. You deserve a space where you feel truly seen and understood.
  phone: 368-996-6230
  email: info@gystservices.ca
  hours: Mon–Thu 12 PM – 8 PM
  location: Online counselling across Alberta & British Columbia
  free_session:
    pill: Complimentary
    heading: Your first session is free
    body: Good fit matters. That's why your initial session comes at no charge — so we can both feel confident about working together. Book by phone or email and we'll find a time that works.
---
